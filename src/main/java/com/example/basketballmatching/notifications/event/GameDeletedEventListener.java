...
@Slf4j
public class GameDeletedEventListener {
    ...
    public void handle(GameDeletedEvent event) {
        ...
        for (Long receiverId : event.receiverIds()) {
            ...
            if (receiver == null) {
                log.warn("경기 삭제 알림 수신자 없음: gameId={}, receiverId={}", event.gameId(), receiverId);

                continue;
            }

            notificationService.send(NotificationType.DELETE_GAME, receiver, createContent(event));

        }
    }

    private String createContent(GameDeletedEvent event) {
        return "'" +event.gameTitle() + "' 경기가 삭제되었습니다.";
    }
}
